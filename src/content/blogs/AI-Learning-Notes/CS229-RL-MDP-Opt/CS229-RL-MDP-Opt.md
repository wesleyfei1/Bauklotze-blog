---
title: 'CS229  强化学习：MDP以及其优化方法'
publishDate: 2026-01-11
updatedDate: 2026-01-11
description: 'CS229  强化学习：MDP以及其优化方法'
category: tech
tags:
  - cs229
  - rl
...
$L(S,A)=R(s_0,a_0)+R(s_1,a_1)+R(s_2,a_2)+....$

考虑到时间的因素，我们对于 $s_1,s_2$ 都是假设之后的状态以及做出的动作，应该加上一个因子来衡量时间的影响

$L(S,A)=R(s_0,a_0)+\gamma R(s_1,a_1)+\gamma^2 R(s_2,a_2)+....$

常常我们的损失只会与我们的状态有关，比如一个机器人如果倒地了那么就将损失定为-1，否则为0。而对于 $a_0$
...
对于 $s_0$ 处的损失取平均值，有 $L(S)=E(R(s_0)+\gamma R(s_1)+\gamma^2 R(s_2)+\gamma^3 R(s_3)...)$ 我们希望得到一个最合适的策略 $a=\pi(s)$  对于之后的步骤选取我们都使用这个策略，于是有定义
...
我们的策略是希望这个策略让损失最小，如果有 $V^*(s) = \max_{\pi} V^\pi(s)$ 的话,那么有 $V^*(s) = R(s) + \max_{a \in A} \gamma \sum_{s' \in S} P_{sa}(s') V^*(s')$ 该方程也称为**Bellman equation**

有了对于每一个步骤的评价函数，那么对于** 策略的选取**为

$a^*=\pi^*(s) =\arg\max_{a\in A} V(s)= \arg\max_{a \in A} \sum_{s' \in S} P_{sa}(s') V^*(s')$

以上是数学上的一个推导，而我们对于具体的计算的话，我们发现 $V^*(s)$

解的话为一个**线性方程组** ，如果直接求解会比较的慢，实际的使用中，一种常见的方法为**value iteration**
...
数学表达为 $ \boxed{P_{sa}(s') \leftarrow \frac{N(s, a, s')}{\sum_{s''} N(s, a, s'')}} $
...
以上是**最为基本的MDP**,他纯纯使用** 贪心**算法，而且我们的操作是只有** 离散**的情形。而在此基础上还可以进行一定的改进
...
如果我们使用MDP的话，会直接通往+1的点，但是实际的话** 贪心并不是最优的策略**,于是我们对于动作的选取方案改变为： $a=\eta \pi(s)+ (1-\eta)a_{randomly choosen}$
...
（1）对于每一个i，计算在每一个动作a下面的损失 $q(a) = \frac{1}{k} \sum_{j=1}^{k} R(s^{(i)}) + \gamma V(s'_j)$ (将q作为对于 $V(s^i)$
...
以上变为强化学习的**最为基本** 的内容，对于具体的代码实现(我们已经给出了伪代码)请参见github仓库，MDP(使用贪心进行策略选取，并以此优化策略)的方法还是**非常的精妙的** ，更多的强化学习的内容之后我们会在cs236中进行学习。
